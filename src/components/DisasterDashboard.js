...
import { disasterData, economicData } from '../data/disasterData';
...
const DisasterDashboard = () => {
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto bg-gray-800 text-white border-0 shadow-none p-0">
  ...
  <CardContent className="bg-gray-800 text-white p-0">
    <ResponsiveContainer width="100%" height={400}>
      <ComposedChart data={data} onClick={handleBarClick}>
        ...
        <YAxis
          ...
          orientation="left"
          stroke="#fff"
          label={{ value: 'Number of Deaths', angle: -90, position: 'insideLeft', fill: '#fff' }}
        />
        <YAxis
          yAxisId="right"
          orientation="right"
          ...
          label={{ value: 'Cost in Crores (INR)', angle: 90, position: 'insideRight', fill: '#82ca9d' }}
        />
        <Tooltip 
  ...
            <Bar key={key} dataKey={key} fill={colors[key]} yAxisId="left" stackId="a" stroke="none" />
        ))}
